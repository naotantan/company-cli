...
interface Plugin {
  ...
  usage_examples_translated?: string | null;
  usage_count?: number;
...
/** 最終使用日を相対表記で返す（i18nなし版 — PluginCard外から呼ばれる互換用） */
...
export function DescriptionBlock({ desc, onClick, t }: { desc: string; onClick: () => void; t: (key: string) => string }) {
...
export function PluginCard({
  ...
  // 翻訳済み examples を優先、なければ原文、なければクライアント側で抽出
  const parseExamples = (json: string | null | undefined): string[] => {
    try { return json ? JSON.parse(json) as string[] : []; } catch { return []; }
  };
  const samplePrompts: string[] = plugin.usage_examples_translated
    ? parseExamples(plugin.usage_examples_translated)
    : plugin.usage_examples
      ? parseExamples(plugin.usage_examples)
  ...
  const handleCopyPrompt = (prompt: string, e: React.MouseEvent) => {
    ...
    const textToCopy = plugin.trigger_type !== 'auto'
    ...
    <div className="bg-th-surface-0 rounded-th p-4 border border-th-border hover:border-th-border-accent transition-all duration-150 flex flex-col gap-2">
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap">
          ...
          {plugin.trigger_type === 'auto' ? (
            ...
            <span
              ...
              onClick={(e) => {
                ...
                el.textContent = t('plugins.copiedPrompt');
                setTimeout(() => { if (el.textContent === t('plugins.copiedPrompt')) el.textContent = orig; }, 1200);
              }}
              title={t('plugins.clickToCopy')}
            >
        ...
        <span className="text-xs text-th-text-4 flex items-center gap-1 shrink-0">
          ...
          {t('plugins.usageCount', { count: plugin.usage_count ?? 0 })}
        </span>
      </div>

      {/* 説明文 */}
      {desc && (
        <DescriptionBlock desc={desc} onClick={() => onShowDetail(plugin)} t={t} />
      )}

      {/* サンプルプロンプト */}
      {samplePrompts.length > 0 && (
        <div className="space-y-1 border-t border-th-border/50 pt-2 mt-1">
          <p className="text-xs text-th-text-4">{t('plugins.usageExamples')} <span className="text-th-text-4/60">{t('plugins.clickToCopy')}</span></p>
          {samplePrompts.map((prompt, i) => {
            const invocationName = plugin.name.replace(/[^a-zA-Z0-9_.-]/g, '');
            const isManual = plugin.trigger_type !== 'auto';
            const displayText = isManual ? `/${invocationName} ${prompt}` : prompt;
            return (
              <button
                key={i}
                ...
                title={displayText}
              >
                {copiedPrompt === prompt ? t('plugins.copiedPrompt') : `"${displayText}"`}
              </button>
            );
          })}
        </div>
      ...
      <div className="flex items-center justify-between gap-2 mt-auto pt-1 border-t border-th-border/40">
        <span className="text-xs text-th-text-4">
          {lastUsed ? t('plugins.lastUsedLabel', { value: lastUsed }) : t('plugins.neverUsed')}
        </span>
        ...
        >
          {t('plugins.viewDetail')}
        </button>
        <button
          onClick={(e) => { e.stopPropagation(); onToggleFavorite(plugin.id); }}
          aria-label={favorites.has(plugin.id) ? t('plugins.removeFromFavorites') : t('plugins.addToFavorites')}
          ...
          title={favorites.has(plugin.id) ? t('plugins.removeFromFavorites') : t('plugins.addToFavorites')}
        >
