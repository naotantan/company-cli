...
Claude Code（および Claude API）を使って AI エージェントを日々の業務に導入していくと、「誰がどのスキルを使っているか分からない」「API コストが把握できない」「セッションの作業記録が散逸する」といった運用上の課題が出てきます。
...
- Plane などのプロジェクト管理ツールと **連携**
- REST API・Web ダッシュボード の **2 インターフェース** に対応
...
- **メトリクスチップ**：セッション数・アクティブジョブ・今月のコストを一覧表示
- **セッションフィード**：最新セッションのリアルタイム表示
- **ジョブパネル**：実行中・完了済みジョブの状態管理
- **スキル使用チャート**：スキルごとの使用頻度を可視化
- **コストサマリー**：期間別コスト推移
...
> **プロジェクト・Issue 管理は Plane を使用**
> maestro は日常のエージェント運用に特化しています。開発タスクや Issue 管理は [Plane](https://plane.so) などの外部ツールとの連携を推奨します。
...
Skill ツール実行時に自動で使用回数・最終使用日を記録します。
...
設定画面で Plane の接続情報を入力することで、外部の Plane プロジェクトと連携できます。maestro の日常運用ログと Plane の開発タスク管理を組み合わせたハイブリッド運用が可能です。
